...
class infinite_mixture_unigram(object):
    ...
    def compute_vector(
        ...
        self.topic_dist = (
            np.sum(self.counter_docs[:, activated_topics], axis=0) + self.alpha
        ) / (
            self.counter_docs[:, activated_topics].sum()
            + self.alpha * len(activated_topics)
        )
        topic_aware_words_dist = (
            self.counter_words[:, activated_topics] + self.beta
        ) / (
            np.sum(self.counter_words[:, activated_topics], axis=0)
            + self.beta * self.n_dims[1]
        )
...
def _gibbs_sampling_CRP(
    X, Z, k_index, k_last, counter_docs, counter_words, alpha, beta, random_state
):
    ...
    for doc_ind, doc_words_freq in tqdm(enumerate(X), total=X.shape[0]):
        ...
        posts += gammaln(
            np.sum(counter_words, axis=0) + beta * nunique_words
        ) - gammaln(np.sum(counter_words, axis=0) + doc_nwords + beta * nunique_words)
        for word_ind, w_freq in enumerate(doc_words_freq):
            if w_freq > 0:
                ...
                posts += gammaln(counter_words[word_ind] + w_freq + beta) - gammaln(
                    counter_words[word_ind] + beta
                )
        ...
        posts[add_topic] += gammaln(beta * nunique_words) - gammaln(
            doc_nwords + beta * nunique_words
        )
        for word_ind, w_freq in enumerate(doc_words_freq):
            if w_freq > 0:
